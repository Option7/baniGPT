...
void handleUpdate(TBMessage msg, String& file_path) {
  ...
  t_httpUpdate_return ret = UPDATER.update(client1, file_path);
...
void telegramOtaSetup() {
  ...
  client1.setTrustAnchors(&certificate);
  client1.setBufferSizes(1024, 1024);
#elif defined(ESP32)
  // Sync time with NTP
  configTzTime(MYTZ, "time.google.com", "time.windows.com", "pool.ntp.org");
  client1.setCACert(telegram_cert);
#endif
    ...
    switch (msg.messageType) {
      ...
      default:
        ...
        if (tgReply.equalsIgnoreCase("/version")) {
          ...
          myBot.sendMessage(msg, "HI LEMUEL. Send firmware binary file ###.bin to start update\n"
